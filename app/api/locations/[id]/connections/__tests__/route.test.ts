...
describe("Location Connections API", () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(campaignsStorage.getCampaignsByUserId).mockResolvedValue([
      mockCampaign as unknown as Campaign,
    ]);
  ...
  describe("GET", () => {
    it("should return connections for a location", async () => {
      ...
      vi.mocked(locationsStorage.getLocationConnections).mockResolvedValue(
        mockConnections as unknown as LocationConnection[]
      );
  ...
  describe("POST", () => {
    it("should create a connection if user is GM", async () => {
      ...
      vi.mocked(locationsStorage.getLocation).mockImplementation(async (_cId, lId) => {
        if (lId === locationId || lId === "loc-2")
          return { id: lId, campaignId, name: "Loc" } as unknown as Location;
        return null;
      });
      vi.mocked(locationsStorage.createLocationConnection).mockResolvedValue({
        id: "conn-1",
        ...body,
      ...
      const request = createMockRequest(
        `http://localhost/api/locations/${locationId}/connections`,
        body,
        "POST"
      );
    ...
    it("should return 403 if user is not GM", async () => {
      vi.mocked(campaignsStorage.getCampaignsByUserId).mockResolvedValue([
        { ...mockCampaign, gmId: "other-user" } as unknown as Campaign,
      ]);
      const body = { toLocationId: "loc-2", connectionType: "physical" };
  ...
  describe("DELETE", () => {
    it("should delete a connection if user is GM", async () => {
      const request = createMockRequest(
        `http://localhost/api/locations/${locationId}/connections?connectionId=conn-1`,
        undefined,
        "DELETE"
      );
