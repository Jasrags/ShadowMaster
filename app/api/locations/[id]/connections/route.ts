...
export async function GET(
    ...
    try {
        ...
        if (!userId) {
            return NextResponse.json({ success: false, connections: [], error: "Authentication required" }, { status: 401 });
        }
        ...
        if (!campaignId) {
            return NextResponse.json({ success: false, connections: [], error: "Location not found or access denied" }, { status: 404 });
        }
        ...
        return NextResponse.json({ success: false, connections: [], error: "An error occurred" }, { status: 500 });
    }
}
...
export async function POST(
    ...
    try {
        ...
        if (!isGM) {
            return NextResponse.json({ success: false, error: "Only the GM can create connections" }, { status: 403 });
        }

        // Verify target location exists in the same campaign
        const targetLoc = await getLocation(campaignId, toLocationId);
        if (!targetLoc) {
            return NextResponse.json({ success: false, error: "Target location not found in this campaign" }, { status: 400 });
        }
...
export async function DELETE(
    ...
    try {
        ...
        if (!isGM) {
            return NextResponse.json({ success: false, error: "Only the GM can delete connections" }, { status: 403 });
        }
