/**
 * Tests for /api/characters/[characterId]/inventory endpoint
 *
 * Tests inventory state retrieval and equipment readiness updates.
 */

import { describe, it, expect, vi, beforeEach } from "vitest";
import { GET, PATCH } from "../route";
import { NextRequest } from "next/server";
import * as sessionModule from "@/lib/auth/session";
import * as characterStorageModule from "@/lib/storage/characters";
import * as authorizationModule from "@/lib/auth/character-authorization";

import type { Character, Weapon, ArmorItem } from "@/lib/types";

// Mock dependencies
vi.mock("@/lib/auth/session");
vi.mock("@/lib/storage/characters");
vi.mock("@/lib/auth/character-authorization");

// Helper to create a NextRequest
function createMockRequest(url: string, body?: unknown, method = "GET"): NextRequest {
  const request = new NextRequest(url, {
    method,
    body: body ? JSON.stringify(body) : undefined,
    headers: body ? { "Content-Type": "application/json" } : undefined,
  });

  if (body) {
    (request as { json: () => Promise<unknown> }).json = async () => body;
  }

  return request;
}

// Create mock character with inventory
function createMockCharacter(overrides: Partial<Character> = {}): Character {
  return {
    id: "test-char-id",
    ownerId: "test-user-id",
    editionId: "sr5",
    editionCode: "sr5",
    creationMethodId: "priority",
    rulesetSnapshotId: "snapshot-1",
    name: "Test Character",
    status: "active",
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
    attributes: { strength: 4 },
    weapons: [],
    armor: [],
    gear: [],
    karmaCurrent: 10,
    karmaTotal: 10,
    nuyen: 1000,
    startingNuyen: 5000,
    ...overrides,
  } as Character;
}

describe("GET /api/characters/[characterId]/inventory", () => {
  const userId = "test-user-id";
  const characterId = "test-char-id";
  let mockCharacter: Character;

  beforeEach(() => {
    vi.clearAllMocks();

    mockCharacter = createMockCharacter({
      weapons: [
        {
          id: "weapon-1",
          name: "Ares Predator V",
          damage: "8P",
          ap: -1,
          mode: ["SA"],
          subcategory: "heavy-pistol",
          category: "ranged",
          quantity: 1,
          cost: 725,
          weight: 1.5,
          state: { readiness: "holstered", wirelessEnabled: true },
        } as Weapon,
      ],
      armor: [
        {
          id: "armor-1",
          name: "Armor Jacket",
          armorRating: 12,
          equipped: true,
          category: "armor",
          quantity: 1,
          cost: 1000,
          weight: 2,
          state: { readiness: "worn", wirelessEnabled: true },
        } as ArmorItem,
      ],
    });

    vi.mocked(sessionModule.getSession).mockResolvedValue(userId);
    vi.mocked(authorizationModule.authorizeOwnerAccess).mockResolvedValue({
      authorized: true,
      character: mockCharacter,
      campaign: null,
      role: "owner",
      permissions: ["view", "edit"],
      status: 200,
    });
  });

  it("should return inventory state with encumbrance", async () => {
    const request = createMockRequest(
      `http://localhost:3000/api/characters/${characterId}/inventory`
    );

    const response = await GET(request, { params: Promise.resolve({ characterId }) });
    const data = await response.json();

    expect(data.success).toBe(true);
    expect(data.inventory).toBeDefined();
    expect(data.inventory.weapons).toHaveLength(1);
    expect(data.inventory.armor).toHaveLength(1);
    expect(data.inventory.encumbrance).toBeDefined();
    expect(data.inventory.encumbrance.maxCapacity).toBe(40); // Strength 4 * 10
    expect(data.inventory.summary).toBeDefined();
  });

  it("should return 401 when not authenticated", async () => {
    vi.mocked(sessionModule.getSession).mockResolvedValue(null);

    const request = createMockRequest(
      `http://localhost:3000/api/characters/${characterId}/inventory`
    );

    const response = await GET(request, { params: Promise.resolve({ characterId }) });
    const data = await response.json();

    expect(response.status).toBe(401);
    expect(data.success).toBe(false);
    expect(data.error).toBe("Unauthorized");
  });

  it("should return 404 when character not found", async () => {
    vi.mocked(authorizationModule.authorizeOwnerAccess).mockResolvedValue({
      authorized: false,
      character: null,
      campaign: null,
      role: "owner",
      permissions: [],
      error: "Character not found",
      status: 404,
    });

    const request = createMockRequest(
      `http://localhost:3000/api/characters/${characterId}/inventory`
    );

    const response = await GET(request, { params: Promise.resolve({ characterId }) });
    const data = await response.json();

    expect(response.status).toBe(404);
    expect(data.success).toBe(false);
  });
});

describe("PATCH /api/characters/[characterId]/inventory", () => {
  const userId = "test-user-id";
  const characterId = "test-char-id";
  let mockCharacter: Character;

  beforeEach(() => {
    vi.clearAllMocks();

    mockCharacter = createMockCharacter({
      weapons: [
        {
          id: "weapon-1",
          name: "Ares Predator V",
          damage: "8P",
          ap: -1,
          mode: ["SA"],
          subcategory: "heavy-pistol",
          category: "ranged",
          quantity: 1,
          cost: 725,
          weight: 1.5,
          state: { readiness: "holstered", wirelessEnabled: true },
        } as Weapon,
      ],
    });

    vi.mocked(sessionModule.getSession).mockResolvedValue(userId);
    vi.mocked(authorizationModule.authorizeOwnerAccess).mockResolvedValue({
      authorized: true,
      character: mockCharacter,
      campaign: null,
      role: "owner",
      permissions: ["view", "edit"],
      status: 200,
    });
    vi.mocked(characterStorageModule.updateCharacter).mockResolvedValue(mockCharacter);
  });

  it("should update weapon readiness state", async () => {
    const requestBody = {
      itemId: "weapon-1",
      itemType: "weapon",
      newState: "readied",
    };

    const request = createMockRequest(
      `http://localhost:3000/api/characters/${characterId}/inventory`,
      requestBody,
      "PATCH"
    );

    const response = await PATCH(request, { params: Promise.resolve({ characterId }) });
    const data = await response.json();

    expect(data.success).toBe(true);
    expect(data.result).toBeDefined();
    expect(data.result.newState).toBe("readied");
    expect(data.result.previousState).toBe("holstered");
    expect(data.result.actionCost).toBe("simple");
  });

  it("should return 400 for invalid state transition", async () => {
    // Armor can only be worn or stored
    mockCharacter = createMockCharacter({
      armor: [
        {
          id: "armor-1",
          name: "Armor Jacket",
          armorRating: 12,
          equipped: true,
          category: "armor",
          quantity: 1,
          cost: 1000,
          state: { readiness: "worn", wirelessEnabled: true },
        } as ArmorItem,
      ],
    });

    vi.mocked(authorizationModule.authorizeOwnerAccess).mockResolvedValue({
      authorized: true,
      character: mockCharacter,
      campaign: null,
      role: "owner",
      permissions: ["view", "edit"],
      status: 200,
    });

    const requestBody = {
      itemId: "armor-1",
      itemType: "armor",
      newState: "readied", // Invalid for armor
    };

    const request = createMockRequest(
      `http://localhost:3000/api/characters/${characterId}/inventory`,
      requestBody,
      "PATCH"
    );

    const response = await PATCH(request, { params: Promise.resolve({ characterId }) });
    const data = await response.json();

    expect(data.success).toBe(false);
    expect(data.error).toContain("Invalid state transition");
  });

  it("should return 404 for non-existent item", async () => {
    const requestBody = {
      itemId: "non-existent",
      itemType: "weapon",
      newState: "readied",
    };

    const request = createMockRequest(
      `http://localhost:3000/api/characters/${characterId}/inventory`,
      requestBody,
      "PATCH"
    );

    const response = await PATCH(request, { params: Promise.resolve({ characterId }) });
    const data = await response.json();

    expect(response.status).toBe(404);
    expect(data.success).toBe(false);
    expect(data.error).toContain("Weapon not found");
  });

  it("should return 400 for missing required fields", async () => {
    const requestBody = {
      itemId: "weapon-1",
      // Missing itemType and newState
    };

    const request = createMockRequest(
      `http://localhost:3000/api/characters/${characterId}/inventory`,
      requestBody,
      "PATCH"
    );

    const response = await PATCH(request, { params: Promise.resolve({ characterId }) });
    const data = await response.json();

    expect(response.status).toBe(400);
    expect(data.success).toBe(false);
    expect(data.error).toContain("Missing required fields");
  });
});
