/**
 * Tests for /api/campaigns/[id]/advancements/[recordId]/reject endpoint
 *
 * Tests advancement rejection (POST) functionality for GMs.
 */
...
describe("POST /api/campaigns/[id]/advancements/[recordId]/reject", () => {
  beforeEach(() => vi.clearAllMocks());

  it("should return 401 when not authenticated", async () => {
    vi.mocked(sessionModule.getSession).mockResolvedValue(null);
    const request = createMockRequest(
      "http://localhost:3000/api/campaigns/test-campaign-id/advancements/test-record-id/reject",
      { characterId: "test-character-id", reason: "Does not meet requirements" }
    );
  ...
  it("should reject advancement successfully", async () => {
    const mockCampaign = createMockCampaign();
    const mockCharacter = createMockCharacter();
    const rejectedCharacter = { ...mockCharacter };
    ...
    vi.mocked(advancementApproval.rejectAdvancement).mockReturnValue({
      updatedCharacter: rejectedCharacter,
      updatedAdvancementRecord: {
        ...mockCharacter.advancementHistory![0],
        gmApproved: false,
        rejectionReason: "Does not meet requirements",
      },
    });
    vi.mocked(characterStorage.updateCharacter).mockResolvedValue(rejectedCharacter);
    const request = createMockRequest(
      "http://localhost:3000/api/campaigns/test-campaign-id/advancements/test-record-id/reject",
      { characterId: "test-character-id", reason: "Does not meet requirements" }
    );
    ...
    expect(advancementApproval.rejectAdvancement).toHaveBeenCalledWith(
      mockCharacter,
      "test-record-id",
      "test-gm-id",
  ...
  it("should return 404 when campaign not found", async () => {
    ...
    const request = createMockRequest(
      "http://localhost:3000/api/campaigns/nonexistent/advancements/test-record-id/reject",
      { characterId: "test-character-id", reason: "Does not meet requirements" }
    );
    const response = await POST(request, {
  ...
  it("should return 404 when character not found", async () => {
    ...
    const request = createMockRequest(
      "http://localhost:3000/api/campaigns/test-campaign-id/advancements/test-record-id/reject",
      { characterId: "nonexistent-character", reason: "Does not meet requirements" }
    );
    const response = await POST(request, {
      params: Promise.resolve({ id: "test-campaign-id", recordId: "test-record-id" }),
    });
