/**
 * Tests for /api/campaigns/[id]/advancements endpoint
 *
 * Tests pending advancement listing (GET) functionality for GMs.
 */
...
describe("GET /api/campaigns/[id]/advancements", () => {
  ...
  it("should return pending advancements for GM", async () => {
    const mockCampaign = createMockCampaign();
    const mockCharacter = createMockCharacter({
      advancementHistory: [
        createMockAdvancementRecord({ gmApproved: false }),
        createMockAdvancementRecord({ id: "approved-record", gmApproved: true }),
      ],
    });
    ...
    expect(data.advancements).toBeDefined();
  });
  ...
  it("should return empty array when no pending advancements", async () => {
    ...
    const data = await response.json();
    expect(response.status).toBe(200);
    expect(data.advancements).toEqual([]);
  });
  ...
  it("should return 404 when campaign not found", async () => {
    ...
    const request = createMockRequest(
      "http://localhost:3000/api/campaigns/nonexistent/advancements"
    );
