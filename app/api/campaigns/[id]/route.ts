...
import { updateCampaign, deleteCampaign } from "@/lib/storage/campaigns";
import { authorizeCampaign } from "@/lib/auth/campaign";
import type { UpdateCampaignRequest, CampaignResponse, Campaign } from "@/lib/types";
...
export async function GET(
  ...
  try {
    ...
    const {
      ...
      error,
      status,
    } = await authorizeCampaign(id, userId, { allowPublic: true });
...
export async function PUT(
  ...
  try {
    ...
    if (body.advancementSettings) {
      ...
      if (
        (settings.attributeKarmaMultiplier !== undefined &&
          settings.attributeKarmaMultiplier < 1) ||
      ...
      return NextResponse.json(
        {
          success: false,
          error:
            "Edition cannot be changed after campaign initialization. Create a new campaign for a different edition.",
        },
        { status: 400 }
      );
    }
