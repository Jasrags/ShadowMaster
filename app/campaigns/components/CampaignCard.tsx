    ...
    >
      ...
      <div className="p-6">
        <div className="flex items-start justify-between">
            ...
            {campaign.tags && campaign.tags.length > 0 && (
              <div className="mt-2 flex flex-wrap gap-2">
                {campaign.tags.slice(0, 3).map((tag) => (
                  <span
                    key={tag}
                    className="inline-flex items-center rounded-md bg-zinc-100 px-2 py-0.5 text-xs font-medium text-zinc-800 dark:bg-zinc-800 dark:text-zinc-300"
                  >
                    #{tag}
                  </span>
                ))}
                {campaign.tags.length > 3 && (
                  <span className="text-xs text-zinc-500">
                    +<span className="font-mono">{campaign.tags.length - 3}</span>
                  </span>
                )}
              </div>
            )}
          ...
          <span
            className={`ml-4 inline-flex items-center rounded-full px-2 py-1 text-xs font-medium ${statusColors[campaign.status]}`}
          >
