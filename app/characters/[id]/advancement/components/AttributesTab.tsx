...
export function AttributesTab({ character, ruleset, onCharacterUpdate }: AttributesTabProps) {
  ...
  const renderAttribute = (attrId: string) => {
    ...
    const cost = canAdvance
      ? attrId === "edge"
        ? calculateEdgeCost(nextRating)
        : calculateAttributeCost(nextRating)
      : 0;
        ...
        <div className="flex items-center justify-between">
          <div className="flex-1">
            <div className="flex items-center gap-3">
              <span className="font-medium text-zinc-100">{ATTRIBUTE_NAMES[attrId] || attrId}</span>
          ...
          {!canAdvance && <span className="text-xs text-zinc-500">Max rating</span>}
        </div>
      </div>
    );
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="space-y-3">{PHYSICAL_ATTRIBUTES.map(renderAttribute)}</div>
      </div>
      ...
      <div>
        ...
        <div className="space-y-3">{MENTAL_ATTRIBUTES.map(renderAttribute)}</div>
      </div>
      ...
      <div>
        ...
        <div className="space-y-3">{renderAttribute("edge")}</div>
      </div>
                    ...
                    <div className="p-3 bg-zinc-800 rounded border border-zinc-700">
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="font-bold text-amber-400">
                          {newRating
                            ? selectedAttribute === "edge"
                              ? calculateEdgeCost(newRating)
                              : calculateAttributeCost(newRating)
                            : 0}
                        </span>
                      </div>
