    ...
    return (
      <div className="space-y-4">
        <div className="flex items-center gap-2 px-1">
          <div
            className={`h-1.5 w-1.5 rounded-full ${isPositive ? "bg-emerald-500" : "bg-red-500"}`}
          />
          <span
            className={`text-[10px] font-bold uppercase tracking-widest ${t.id === "modern-card" ? "text-zinc-500" : "text-muted-foreground"}`}
          >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 px-0.5">
          {selections.map((selection) => {
            const id =
              typeof selection === "string" ? selection : selection.qualityId || selection.id || "";
            const data = (
              isPositive
                ? positiveData.find((q: QualityData) => q.id === id)
                : negativeData.find((q: QualityData) => q.id === id)
            ) as QualityData | undefined;
            ...
            const qualityLevels =
              ...
              "qualityLevels" in stateSelections
            ...
            const qualitySpecifications =
              ...
              "qualitySpecifications" in stateSelections
                ? (stateSelections.qualitySpecifications as
                  ...
                  <div className="mt-2 pt-2 border-t border-border/30">
                    {rawSelection.dynamicState.type === "addiction" && (
                      <div className="flex items-center gap-1.5 text-[9px] text-amber-500 dark:text-amber-400 font-medium">
                        ...
                        <span>
                          {rawSelection.dynamicState.state.severity.toUpperCase()} •{" "}
                          {rawSelection.dynamicState.state.substance} (
                          {rawSelection.dynamicState.state.substanceType})
                        </span>
                      </div>
                    )}
                    {rawSelection.dynamicState.type === "allergy" && (
                      <div className="flex items-center gap-1.5 text-[9px] text-amber-500 dark:text-amber-400 font-medium">
                        ...
                        <span>
                          {rawSelection.dynamicState.state.severity.toUpperCase()} •{" "}
                          {rawSelection.dynamicState.state.allergen} (
                          {rawSelection.dynamicState.state.prevalence})
                        </span>
                      </div>
                    )}
                    {rawSelection.dynamicState.type === "dependent" && (
                      <div className="flex items-center gap-1.5 text-[9px] text-amber-500 dark:text-amber-400 font-medium">
                        <AlertCircle className="w-2.5 h-2.5" />
                        <span>
                          TIER {rawSelection.dynamicState.state.tier} •{" "}
                          {rawSelection.dynamicState.state.relationship} (
                          {rawSelection.dynamicState.state.currentStatus})
                        </span>
                      </div>
                    )}
                  </div>
