    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between">
        <div className="flex gap-3">
          <div
            className={`p-2 rounded bg-muted/40 ${state.currentlyExposed ? "text-amber-500 animate-pulse" : "text-muted-foreground"}`}
          >
            <Wind className="w-5 h-5" />
          </div>
          <div>
            <h4 className="text-sm font-bold text-foreground">{state.allergen} Allergy</h4>
            <p
              className={`text-xs mt-0.5 uppercase tracking-wider font-mono font-bold ${getSeverityColor(state.severity)}`}
            >
        ...
        <div className="text-right">
          ...
          <div
            className={`text-xl font-mono font-bold ${state.damageAccumulated > 0 ? "text-red-500" : "text-muted-foreground"}`}
          >
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="text-left">
            ...
            <div className="text-[9px] opacity-70 italic font-mono">
              {state.currentlyExposed
                ? `Active since ${new Date(state.exposureStartTime!).toLocaleTimeString()}`
                : "Not currently exposed"}
            </div>
          </div>
        </div>
        <div
          className={`w-10 h-5 rounded-full p-1 transition-colors ${state.currentlyExposed ? "bg-amber-500" : "bg-muted-foreground/30"}`}
        >
          <div
            className={`w-3 h-3 bg-white rounded-full transition-transform ${state.currentlyExposed ? "translate-x-5" : "translate-x-0"}`}
          />
        </div>
      </button>
      ...
      <div className="p-3 bg-blue-500/5 border border-blue-500/10 rounded flex gap-3 items-start">
        ...
        <p className="text-[10px] text-blue-600/80 dark:text-blue-400/80 italic leading-relaxed">
          ...
          {state.severity === "moderate" &&
            "Exposure applies a -4 dice pool modifier for all actions."}
          {state.severity === "severe" &&
            "Exposure applies a -4 dice pool modifier and deals 1 box of Physical damage per minute."}
          {state.severity === "extreme" &&
            "Exposure applies a -6 dice pool modifier and deals 1 box of Physical damage per 30 seconds."}
        </p>
      </div>
    </div>
  );
