...
export function BudgetIndicator({
      ...
      <div className={className}>
        <div className="flex items-center justify-between gap-2">
          ...
          <span className={`text-xs font-medium ${getStatusColor()}`}>
            {formatValue(spent)} spent
            <span className="text-zinc-400"> • </span>
            {formatValue(Math.max(0, remaining))} left
            {hasOverflow && (
              <span className="text-red-500"> (+{formatValue(spent - total)})</span>
            )}
          </span>
        </div>
        {note && (
          <div className={`mt-0.5 text-right text-[10px] ${getNoteColor()}`}>
            {note}
          </div>
        )}
      </div>
  ...
  return (
    <div className={className}>
      ...
      {note && (
        <div className={`mt-1 text-xs ${getNoteColor()}`}>
          {note}
        </div>
      )}
    </div>
