...
export function CyberdeckConfig({
  ...
  const handleAttrClick = useCallback((attr: AttributeKey) => {
    if (readOnly) return;
  ...
  // Quick swap presets
  const applyPreset = useCallback((preset: 'balanced' | 'offensive' | 'stealthy' | 'defensive') => {
    if (readOnly) return;
  ...
  return (
    <div className={`deck-config ${className}`}>
      <div className="deck-config__header">
        <h4 className="deck-config__title">
      ...
      <div className="deck-config__array">
        ...
        <span className="deck-config__array-values">
          [{deck.attributeArray.join(', ')}]
        </span>
      </div>
