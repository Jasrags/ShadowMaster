...
describe("mergeRules", () => {
  ...
  describe("merge strategy", () => {
    ...
    it("should merge arrays by ID when items have IDs", () => {
      ...
      expect(
        (result.items as Array<{ id: string; value: string; extra?: string }>).find(
          (i) => i.id === "1"
        )
      ).toEqual({
        id: "1",
      ...
      expect(
        (result.items as Array<{ id: string; value: string }>).find((i) => i.id === "2")
      ).toEqual({
        id: "2",
        value: "b",
      });
      expect(
        (result.items as Array<{ id: string; value: string }>).find((i) => i.id === "3")
      ).toEqual({
        id: "3",
        value: "c",
      });
  ...
  describe("append strategy", () => {
    ...
    it("should merge arrays by ID when appending", () => {
      ...
      expect(result.items).toHaveLength(3);
      expect(
        (result.items as Array<{ id: string; value: string; newProp?: string }>).find(
          (i) => i.id === "1"
        )
      ).toEqual({
        id: "1",
  ...
  describe("edge cases", () => {
    ...
    it("should handle mixed array types", () => {
      const base = {
        mixed: [{ id: "1", value: "a" }, "primitive", 42],
      };
      const override = {
        mixed: [{ id: "1", value: "A" }, "new-primitive"],
      };
