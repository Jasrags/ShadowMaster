...
describe("Karma Cost Calculations", () => {
  ...
  describe("calculateAdvancementCost", () => {
    ...
    it("should throw error when rating required but not provided", () => {
      expect(() => calculateAdvancementCost("attribute")).toThrow(
        "Rating required for attribute/edge advancement"
      );
      expect(() => calculateAdvancementCost("skill")).toThrow(
        "Rating required for skill advancement"
      );
      expect(() => calculateAdvancementCost("skillGroup")).toThrow(
        "Rating required for skill group advancement"
      );
      expect(() => calculateAdvancementCost("knowledgeSkill")).toThrow(
        "Rating required for knowledge/language skill advancement"
      );
    });

    it("should throw error for unsupported advancement types", () => {
      expect(() => calculateAdvancementCost("focus" as AdvancementType)).toThrow(
        "Focus bonding cost calculation requires focus type"
      );
      expect(() => calculateAdvancementCost("initiation" as AdvancementType)).toThrow(
        "Initiation cost calculation requires grade"
      );
      expect(() => calculateAdvancementCost("quality" as AdvancementType)).toThrow(
        "Quality costs are handled by quality advancement module"
      );
      // Test with invalid type - need to cast to bypass type checking for this test
      expect(() => calculateAdvancementCost("unknown" as AdvancementType)).toThrow(
