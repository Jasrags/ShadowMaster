...
describe('Training Time Calculations', () => {
  describe('calculateAttributeTrainingTime', () => {
    ...
    it('should throw error for invalid rating', () => {
      expect(() => calculateAttributeTrainingTime(0)).toThrow('Attribute rating must be at least 1');
      expect(() => calculateAttributeTrainingTime(-1)).toThrow('Attribute rating must be at least 1');
    });
  });
  ...
  describe('calculateSkillGroupTrainingTime', () => {
    ...
    it('should throw error for invalid rating', () => {
      expect(() => calculateSkillGroupTrainingTime(0)).toThrow('Skill group rating must be at least 1');
      expect(() => calculateSkillGroupTrainingTime(-1)).toThrow('Skill group rating must be at least 1');
    });
  });
  ...
  describe('calculateFinalTrainingTime', () => {
    ...
    it('should apply instructor bonus first, then time modifier', () => {
      ...
      expect(calculateFinalTrainingTime(100, { instructorBonus: true, timeModifier: 50 })).toBe(113);
  ...
  describe('calculateAdvancementTrainingTime', () => {
    ...
    it('should apply both instructor bonus and time modifier', () => {
      ...
      expect(calculateAdvancementTrainingTime('attribute', 4, { instructorBonus: true, timeModifier: 50 })).toBe(32);
    });

    it('should throw error when rating required but not provided', () => {
      expect(() => calculateAdvancementTrainingTime('attribute')).toThrow('Rating required for attribute training time');
      expect(() => calculateAdvancementTrainingTime('skill')).toThrow('Rating required for skill training time');
      expect(() => calculateAdvancementTrainingTime('skillGroup')).toThrow('Rating required for skill group training time');
    });

    it('should throw error for unsupported advancement types', () => {
      expect(() => calculateAdvancementTrainingTime('spell' as AdvancementType)).toThrow('Spell/ritual training time calculation not yet implemented');
      expect(() => calculateAdvancementTrainingTime('complexForm' as AdvancementType)).toThrow('Complex form training time calculation not yet implemented');
      expect(() => calculateAdvancementTrainingTime('focus' as AdvancementType)).toThrow('Focus bonding training time calculation not yet implemented');
      expect(() => calculateAdvancementTrainingTime('initiation' as AdvancementType)).toThrow('Initiation training time calculation not yet implemented');
      // Test with invalid type - need to cast to bypass type checking for this test
      expect(() => calculateAdvancementTrainingTime('unknown' as AdvancementType)).toThrow('Unknown advancement type: unknown');
    });
  });
});
