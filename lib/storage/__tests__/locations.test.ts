...
describe("Location Storage", () => {
  ...
  afterEach(async () => {
    ...
    try {
      ...
      const campaignDir1 = path.join(process.cwd(), "data", "campaigns", campaignId1);
      const campaignDir2 = path.join(process.cwd(), "data", "campaigns", campaignId2);
  ...
  describe("updateLocation", () => {
    ...
    it("should throw error for non-existent location", async () => {
      await expect(updateLocation(campaignId1, "nonexistent", { name: "Test" })).rejects.toThrow();
    });
  });
  ...
  describe("linkContentToLocation", () => {
    it("should link NPC to location", async () => {
      ...
      const npcId = "npc-123";
      const updated = await linkContentToLocation(campaignId1, location.id, "npc", npcId);
  ...
  describe("unlinkContentFromLocation", () => {
    it("should unlink content from location", async () => {
      ...
      const npcId = "npc-123";
      await linkContentToLocation(campaignId1, location.id, "npc", npcId);
      const updated = await unlinkContentFromLocation(campaignId1, location.id, "npc", npcId);
  ...
  describe("recordLocationVisit", () => {
    it("should record visit and update tracking", async () => {
      ...
      const updated = await recordLocationVisit(campaignId1, location.id, characterId, sessionId);
    ...
    it("should increment visit count on multiple visits", async () => {
      ...
      const updated = await recordLocationVisit(campaignId1, location.id, "char-1");
