...
def convert_dict_to_go(d, context=""):
    ...
    if context == "bonus" or any(k in d for k in ['limitmodifier', 'skillcategory', 'specificskill', 'skillgroup',
        ...
        return convert_cyberware_bonus_to_go(d)
    ...
    # Handle SkillCategoryBonus (for cyberware, use CyberwareSkillCategoryBonus)
    ...
    # Handle CyberwareRequired
    if 'oneof' in d or 'allof' in d:
        return convert_cyberware_required_to_go(d)
...
def convert_cyberware_bonus_to_go(d):
    """Convert cyberware bonus dictionary to Go CyberwareBonus struct"""
    parts = []
    ...
    # Handle skillsoftaccess (cyberware-specific)
    if 'skillsoftaccess' in d:
        ssa = d.get('skillsoftaccess')
        if isinstance(ssa, dict):
            # Can have +content and +@precedence
            content = ssa.get('+content', '')
            precedence = ssa.get('+@precedence', '')
            if content:
                parts.append(f"SkillSoftAccess: {convert_value(content, 'skillsoftaccess')}")
            # Note: precedence is not in the struct, so we skip it
        elif isinstance(ssa, str):
            parts.append(f"SkillSoftAccess: {convert_value(ssa, 'skillsoftaccess')}")
        else:
            # Handle as interface{} for complex cases
            parts.append(f"SkillSoftAccess: nil")  # TODO: Handle complex skillsoftaccess
    ...
    if 'oneof' in d:
        ...
        if 'quality' in oo:
            ...
            if isinstance(q, list):
                # Take first quality if it's a list
                q = q[0] if q else ""
...
def convert_cyberware_required_to_go(d):
    """Convert cyberware required dictionary to Go CyberwareRequired struct"""
    parts = []
    if 'oneof' in d:
        oo = d.get('oneof')
        oo_parts = []
        ...
        if 'cyberware' in oo:
            bw = oo.get('cyberware')
            if isinstance(bw, list):
                items = [convert_value(item, 'cyberware') for item in bw]
                brace_open = '{'
                brace_close = '}'
                oo_parts.append(f"Cyberware: []interface{brace_open}{brace_close}{brace_open}{', '.join(items)}{brace_close}")
            else:
                oo_parts.append(f"Cyberware: {convert_value(bw, 'cyberware')}")
        if 'metatype' in oo:
        ...
        parts.append(f"OneOf: &CyberwareRequiredOneOf{{\n\t\t\t\t\t{', '.join(oo_parts)},\n\t\t\t\t}}")
    if 'allof' in d:
        ao = d.get('allof')
        ao_parts = []
        if 'metatype' in ao:
            ao_parts.append(f"Metatype: {convert_value(ao.get('metatype'), 'metatype')}")
        parts.append(f"AllOf: &CyberwareRequiredAllOf{{\n\t\t\t\t\t{', '.join(ao_parts)},\n\t\t\t\t}}")
    if not parts:
        return "nil"
    return f"&CyberwareRequired{{\n\t\t\t\t{', '.join(parts)},\n\t\t\t}}"
...
def convert_cyberware_to_go(cyberware):
    """Convert cyberware JSON to Go struct"""
    parts = []
    
    # Required fields
    parts.append(f'Name: {convert_value(cyberware.get("name"), "name")}')
    parts.append(f'Category: {convert_value(cyberware.get("category"), "category")}')
    parts.append(f'Ess: {convert_value(cyberware.get("ess"), "ess")}')
    parts.append(f'Capacity: {convert_value(cyberware.get("capacity"), "capacity")}')
    parts.append(f'Avail: {convert_value(cyberware.get("avail"), "avail")}')
    parts.append(f'Cost: {convert_value(cyberware.get("cost"), "cost")}')
    parts.append(f'Source: {convert_value(cyberware.get("source"), "source")}')
    
    # Optional fields
    if 'rating' in cyberware:
        rating = cyberware.get("rating")
    ...
    if 'limit' in cyberware:
        parts.append(f'Limit: {convert_value(cyberware.get("limit"), "limit")}')
    if 'bonus' in cyberware:
        bonus = cyberware.get("bonus")
        if bonus:
            parts.append(f'Bonus: {convert_cyberware_bonus_to_go(bonus)}')
    if 'forbidden' in cyberware:
        parts.append(f'Forbidden: {convert_forbidden_to_go(cyberware.get("forbidden"))}')
    if 'bannedgrades' in cyberware:
        parts.append(f'BannedGrades: {convert_dict_to_go(cyberware.get("bannedgrades"), "bannedgrades")}')
    if 'required' in cyberware:
        req = cyberware.get("required")
        if req:
            parts.append(f'Required: {convert_cyberware_required_to_go(req)}')
    if 'allowgear' in cyberware:
        ag = cyberware.get("allowgear")
        if ag and 'gearcategory' in ag:
            gc = ag.get('gearcategory')
            if isinstance(gc, list):
                items = [convert_value(item, 'gearcategory') for item in gc]
                brace_open = '{'
                brace_close = '}'
                parts.append(f'AllowGear: &AllowGear{{\n\t\t\t\t\tGearCategory: []string{brace_open}{", ".join(items)}{brace_close},\n\t\t\t\t}}')
            else:
                brace_open = '{'
                brace_close = '}'
                parts.append(f'AllowGear: &AllowGear{{\n\t\t\t\t\tGearCategory: []string{brace_open}{convert_value(gc, "gearcategory")}{brace_close},\n\t\t\t\t}}')
        else:
            parts.append('AllowGear: nil')
    if 'allowsubsystems' in cyberware:
        ass = cyberware.get("allowsubsystems")
        if ass and 'category' in ass:
            cat = ass.get('category')
            if isinstance(cat, list):
                # Take first category if it's a list
                cat = cat[0] if cat else ""
            parts.append(f'AllowSubsystems: &AllowSubsystems{{\n\t\t\t\t\tCategory: {convert_value(cat, "category")},\n\t\t\t\t}}')
        else:
            parts.append('AllowSubsystems: nil')
    if 'addweapon' in cyberware:
        addweapon = cyberware.get("addweapon")
        if addweapon is None:
            parts.append('AddWeapon: ""')
        else:
            parts.append(f'AddWeapon: {convert_value(addweapon, "addweapon")}')
    if 'addtoparentess' in cyberware:
        atpe = cyberware.get("addtoparentess")
        if atpe is None:
            brace_open = '{'
            brace_close = '}'
    ...
    if 'requireparent' in cyberware:
        rp = cyberware.get("requireparent")
        if rp is None:
            brace_open = '{'
            brace_close = '}'
    ...
    if 'selectside' in cyberware:
        ss = cyberware.get("selectside")
        if ss is None:
            brace_open = '{'
            brace_close = '}'
    ...
    if 'blocksmounts' in cyberware:
        parts.append(f'BlocksMounts: {convert_value(cyberware.get("blocksmounts"), "blocksmounts")}')
    if 'wirelessbonus' in cyberware:
        wb = cyberware.get("wirelessbonus")
        if wb:
            parts.append(f'WirelessBonus: {convert_cyberware_bonus_to_go(wb)}')
    if 'pairbonus' in cyberware:
        parts.append(f'PairBonus: {convert_pair_bonus_to_go(cyberware.get("pairbonus"))}')
    if 'wirelesspairbonus' in cyberware:
        parts.append(f'WirelessPairBonus: {convert_pair_bonus_to_go(cyberware.get("wirelesspairbonus"))}')
    if 'pairinclude' in cyberware:
        pi = cyberware.get("pairinclude")
        if pi and isinstance(pi, dict):
            name = pi.get('name', '')
            parts.append(f'PairInclude: &PairInclude{{\n\t\t\t\t\tName: {convert_value(name, "name")},\n\t\t\t\t}}')
        else:
            parts.append('PairInclude: nil')
    if 'wirelesspairinclude' in cyberware:
        wpi = cyberware.get("wirelesspairinclude")
        if wpi and isinstance(wpi, dict):
            name = wpi.get('name', '')
            parts.append(f'WirelessPairInclude: &PairInclude{{\n\t\t\t\t\tName: {convert_value(name, "name")},\n\t\t\t\t}}')
        else:
            parts.append('WirelessPairInclude: nil')
    if 'notes' in cyberware:
        parts.append(f'Notes: {convert_value(cyberware.get("notes"), "notes")}')
    if 'forcegrade' in cyberware:
        parts.append(f'ForceGrade: {convert_value(cyberware.get("forcegrade"), "forcegrade")}')
    if 'isgeneware' in cyberware:
        ig = cyberware.get("isgeneware")
        if ig is None:
            brace_open = '{'
            brace_close = '}'
...
# Read JSON file
with open('data/chummer/cyberware.json', 'r') as f:
    data = json.load(f)
...
for category in data['categories']['category']:
    ...
    category_go = f'CyberwareCategory{{\n\t\t\tName: {convert_value(category_name, "name")}, BlackMarket: {convert_value(category.get("+@blackmarket", ""), "blackmarket")},\n\t\t}}'
    category_entries.append(f'\t"{category_id}": {category_go}')

# Generate cyberware entries
cyberware_entries = []
cyberware_ids_seen = {}
for cyberware in data['cyberwares']['cyberware']:
    cyberware_id = to_snake_case(cyberware.get('name', ''))
    # Handle duplicate IDs by appending a suffix
    if cyberware_id in cyberware_ids_seen:
        cyberware_ids_seen[cyberware_id] += 1
        cyberware_id = f"{cyberware_id}_{cyberware_ids_seen[cyberware_id]}"
    else:
        cyberware_ids_seen[cyberware_id] = 0
    cyberware_go = convert_cyberware_to_go(cyberware)
    cyberware_entries.append(f'\t"{cyberware_id}": {cyberware_go}')

# Write to file
with open('cyberware_data_generated.go', 'w') as f:
    f.write('package v5\n\n')
    f.write('// DataGrades contains cyberware grades keyed by their ID (lowercase with underscores)\n')
    f.write('var DataGrades = map[string]Grade{\n')
    f.write(',\n'.join(grade_entries))
    f.write(',\n}\n\n')
    f.write('// DataCyberwareCategories contains cyberware categories keyed by their ID (lowercase with underscores)\n')
    f.write('var DataCyberwareCategories = map[string]CyberwareCategory{\n')
    f.write(',\n'.join(category_entries))
    f.write(',\n}\n\n')
    f.write('// DataCyberwares contains cyberware records keyed by their ID (lowercase with underscores)\n')
    f.write('var DataCyberwares = map[string]Cyberware{\n')
    f.write(',\n'.join(cyberware_entries))
    f.write(',\n}\n')

print(f"Generated {len(grade_entries)} grade entries, {len(category_entries)} category entries, and {len(cyberware_entries)} cyberware entries")
