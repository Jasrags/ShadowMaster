...
export function SourceFilter<T>({ items, selectedSources, onSourcesChange, getSource }: SourceFilterProps<T>) {
  ...
  const sourcesWithCounts = useMemo(() => {
    ...
    items.forEach(item => {
      const source = getSource(item) || 'Unknown';
